...
async function setupConsumidores() {
  ...
  consumerMiembro.run({
    eachMessage: ({ topic, partition, message }) => {
      ...
      (partition == 0)
        ...
        : miembrosMapPremium.set(Patente, { Nombre, Apellido, Rut, Correo, Suscripcion })
    }
  });
  ...
  consumerVentas.run({
    eachMessage: async({ topic, partition, message }) => {
      ...
      } else {console.log("Patente de miembro no registrada, Ventas");}
  }});
